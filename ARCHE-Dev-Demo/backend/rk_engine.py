...
try:
    from arche.sniffers.text.rk_bridge import run_arche_text_sniffer_flags
except Exception:  # pragma: no cover - optional staged dependency path
    run_arche_text_sniffer_flags = None
...
def run_sniffers(message: str) -> list[Flag]:
    ...
    The staged ARCHE text/security sniffer is additive and fail-open: if the
    optional dependency stack is unavailable, the legacy regex path still runs.
    """
    flags: list[Flag] = []
    ...
    if run_arche_text_sniffer_flags is not None:
        try:
            flags.extend(run_arche_text_sniffer_flags(message, source_id="rk_engine"))
        except Exception:
            # The upgraded sniffer is staged as an enhancement. It must never
            # break the legacy demo path while dependencies/config settle.
            pass
...
def public_flag_chips(flags: list[Flag]) -> list[str]:
    ...
    for f in flags:
        ...
        chips.add({
            ...
            0x00A0: "RESOLUTION",
            0x00B0: "RELATIONAL",
            0xA010: "TEXT_SECURITY",
            0xA020: "TEXT_INTENT",
            0xA030: "REPETITION",
            0xA040: "REPETITION",
            0xA0F0: "LOCKDOWN_SIGNAL",
        }.get(prefix, "SIGNAL"))
    return sorted(chips)
