...
function PacedAssistant({ content, delivery, live = false }) {
  const beats = useMemo(() => {
    const deliveryBeats = delivery?.beats || [];
    if (deliveryBeats.length > 0) return deliveryBeats;
    return content ? [content] : [];
  }, [content, delivery]);

  const [visibleBeats, setVisibleBeats] = useState(live ? [] : beats);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const speed = TYPE_SPEED_MS[delivery?.typing_speed || "medium"] || TYPE_SPEED_MS.medium;
    const pause = Math.max(120, Math.min(1600, Number(delivery?.beat_pause_ms || 650)));
    ...
    (async () => {
      for (const beat of beats) {
        ...
        if (cancelled) return;
        setVisibleBeats((prev) => [...prev, beat]);
        setActiveText("");
        await sleep(pause);
      }
  ...
  }, [beats, delivery?.beat_pause_ms, delivery?.typing_speed, live]);

  return (
    <div className="paced-response" data-profile={delivery?.pacing_profile || "calm"}>
      {visibleBeats.map((beat, idx) => (
        <div key={`${idx}-${beat.slice(0, 12)}`} className="paced-beat">
          {beat}
        </div>
      ))}
      {activeText && <div className="paced-beat paced-beat-active">{activeText}</div>}
      {live && !done && <span className="typing-cursor" aria-hidden="true" />}
    </div>
  );
}
...
function Chat() {
      ...
      try {
        ...
        setMessages(
          (d.rk_history || []).flatMap((t) => [
            { role: "user", content: t.user, ts: t.ts || null, live: false },
            { role: "assistant", content: t.assistant, ts: t.ts || null, live: false },
          ])
        );
      } catch (e) {
  ...
  const send = async () => {
    ...
    setMessages((m) => [...m, { role: "user", content: msg, ts: nowIso, live: false }]);
    ...
    try {
      ...
      if (d.rk_response) {
        setMessages((m) => [
          ...
          {
            role: "assistant",
            content: d.rk_response,
            delivery: d.rk_delivery || null,
      ...
      setMessages((m) => [
        ...m,
        { role: "assistant", content: errMsg, ts: new Date().toISOString(), live: true },
      ]);
    } finally {
      setSending(false);
    }
  ...
  const pairs = useMemo(() => {
    ...
    for (const m of messages) {
      if (m.role === "user") {
        ...
        pending = { user: m.content, userTs: m.ts || null, assistant: null, assistantTs: null, assistantDelivery: null, assistantLive: false };
      } else {
        if (pending) {
          pending.assistant = m.content;
          pending.assistantTs = m.ts || null;
          pending.assistantDelivery = m.delivery || null;
          pending.assistantLive = Boolean(m.live);
          out.push(pending);
          pending = null;
        } else {
          out.push({ user: null, userTs: null, assistant: m.content, assistantTs: m.ts || null, assistantDelivery: m.delivery || null, assistantLive: Boolean(m.live) });
        }
      }
    }
    if (pending) out.push(pending);
              ...
              <div key={i} className="pair" data-testid={`pair-${i}`}>
                ...
                {p.assistant && (
                  <div className="bubble bubble-assistant" data-testid={`bubble-assistant-${i}`}>
                    <PacedAssistant
                      content={p.assistant}
                      delivery={p.assistantDelivery}
                      live={p.assistantLive}
                    />
                  </div>
                )}
