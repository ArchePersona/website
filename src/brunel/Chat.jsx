...
function Chat() {
  ...
  const send = async () => {
    ...
    try {
      ...
      const errMsg = `[ link to engine failed — ${e?.message || "unknown"} ]`;
    ...
    <div className="app">
      <div className="topbar">
        ...
        <div className="topbar-right">
          ...
          <button className="reset-btn" onClick={copyThread}><ClipboardCopy size={11} /> {copiedKey === "thread" ? "Copied" : "Copy"}</button>
      ...
      <div className="panel-input shared-seed-box">
        <div className="speech-status">{fileStatus || "free speech input available"}</div>
        ...
        <div className="input-row">
          ...
          <button className="file-btn" onClick={openFilePicker}><Paperclip size={14} /></button>
          <button className={`mic-btn ${listening ? "listening" : ""}`} onClick={toggleSpeech}>{listening ? <MicOff size={14} /> : <Mic size={14} />}</button>
