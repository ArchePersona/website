  ...
  return (
    <div className="app" data-skin={skin}>
      <header className="topbar brunel-shell-topbar">
        <div className="topbar-left"><button className="drawer-toggle cybrary-toggle" onClick={() => openDrawer("library")} title="Cybrary" aria-label="Open Cybrary"><Library size={18} /></button></div>
        <div className="brand brand-centered"><div className="brand-name">BRUNEL</div><div className="brand-sub">The Builder</div></div>
        <div className="topbar-right"><button className="drawer-toggle works-toggle" onClick={() => openDrawer("settings")} title="Settings" aria-label="Open Brunel controls"><Settings size={18} /></button></div>
      </header>
      ...
      <div className="panel-input shared-seed-box">
        {attachedItem && <div className="attachment-chip active"><Paperclip size={14} />{attachedItem.name} · {formatFileSize(attachedItem.size)}<button onClick={() => setAttachedItem(null)} aria-label="Remove attached Cybrary item"><X size={12} /></button></div>}
        {fileStatus && <div className="file-status">{fileStatus}</div>}
        <input ref={fileInputRef} id="brunel-file-upload" name="brunel-file-upload" className="hidden-file-input" type="file" onChange={attachFile} aria-label="Attach file to Cybrary" />
        <div className="input-row"><button className="file-btn" onClick={openFilePicker} disabled={sending} title="Attach" aria-label="Attach file"><Paperclip size={14} /></button><textarea id="brunel-message-input" name="brunel-message-input" className="input" placeholder="What are we working on?" value={text} disabled={sending} onChange={(e) => setText(e.target.value)} onKeyDown={onKey} aria-label="Message BRUNEL" /><button className={`mic-btn ${listening ? "listening" : ""}`} onClick={toggleSpeech} disabled={sending || !speechSupported} title="Voice" aria-label={listening ? "Stop voice input" : "Start voice input"}>{listening ? <MicOff size={14} /> : <Mic size={14} />}</button><button className="send" onClick={send} disabled={sending || (!text.trim() && !attachedItem)} title="Send" aria-label="Send message"><Send size={14} /></button></div>
      </div>

      <div className="footnote"><span>BRUNEL · An ArchePersona product</span><span className="footnote-tag">Powered by ARCHE</span></div>

      {drawerKind && <><div className={`drawer-backdrop ${drawerClosing ? "closing" : "open"}`} onClick={closeDrawer} /><aside className={`drawer ${drawerKind === "library" ? "cybrary-drawer" : "works-drawer"} ${drawerClosing ? "closing" : "open"}`}><div className="drawer-title"><span>{drawerKind === "settings" ? "Brunel Controls" : "Cybrary"}</span><button className="drawer-close" onClick={closeDrawer} aria-label="Close drawer"><X size={18} /></button></div>{drawerKind === "settings" ? <div className="drawer-row"><div className="drawer-section"><span className="drawer-label" id="brunel-skin-label">Skin</span><select className="model-select" value={skin} onChange={(e) => setSkin(e.target.value)} aria-labelledby="brunel-skin-label">{SKINS.map(([v,l]) => <option key={v} value={v}>{l}</option>)}</select></div>{isAdmin && <div className="drawer-section"><span className="drawer-label" id="brunel-model-label">Model</span><select className="model-select" value={selectedModel} onChange={(e) => setSelectedModel(normalizeModelChoice(e.target.value))} aria-labelledby="brunel-model-label">{ADMIN_MODEL_CHOICES.map(([v,l]) => <option key={v} value={v}>{l}</option>)}</select></div>}<div className="drawer-section"><span className="drawer-label">View</span><div className="view-toggle"><button className={viewMode === "single" ? "active" : ""} onClick={() => setViewMode("single")} aria-label="Switch to single view">Single</button><button className={viewMode === "double" ? "active" : ""} onClick={() => setViewMode("double")} aria-label="Switch to dual view">Dual</button></div></div><button className="reset-btn" onClick={startNewThread} disabled={sending} aria-label="Start new chat"><Trash2 size={13} />New Chat</button><button className="reset-btn" onClick={copyThread} disabled={pairs.length === 0} aria-label="Copy transcript"><ClipboardCopy size={13} />{copiedKey === "thread" ? "Copied" : "Copy"}</button>{isAdmin && <button className="reset-btn" onClick={() => navigate("/brunel/admin")} aria-label="Open admin"><Shield size={13} />Admin</button>}<button className="reset-btn" onClick={signOut} aria-label="Exit BRUNEL"><LogOut size={13} />Exit</button></div> : <div className="drawer-row cybrary-list">{cybraryItems.length === 0 ? <p className="drawer-note">No Cybrary items yet.</p> : cybraryItems.map((item) => <button key={item.id} className="cybrary-item" onClick={() => { setAttachedItem(item); closeDrawer(); }} aria-label={`Attach ${item.name} from Cybrary`}><span>{item.name}</span><small>{item.kind || item.type} · {formatFileSize(item.size)} · {item.status || "stored"}</small></button>)}</div>}</aside></>}
    </div>
  );
}
