...
.engine-grid, .engine-controls-grid, .engine-cockpit-grid, .engine-trace-grid { display: grid; grid-template-columns: repeat(12, minmax(0, 1fr)); gap: 10px; }
...
.value-chip, .egress-chip, .engine-pill { display: inline-flex; border: 1px solid rgba(232,255,240,0.14); color: rgba(232,255,240,0.72); background: rgba(255,255,255,0.025); padding: 4px 7px; font-size: 8px; letter-spacing: 0.08em; text-transform: uppercase; }
.value-chip strong { color: #7dffae; margin-left: 6px; }
.egress-chip[data-active='true'], .engine-pill[data-active='true'] { color: #7dffae; border-color: rgba(80,255,150,0.3); background: rgba(80,255,150,0.06); }
.engine-pill[data-active='warn'] { color: #d7b35a; border-color: rgba(215,179,90,0.4); background: rgba(215,179,90,0.06); }
...
.engine-panel-list { display: grid; gap: 8px; }
...
.engine-trace-step { grid-column: span 12; border-left: 2px solid rgba(215,179,90,0.7); padding: 8px 10px; background: rgba(8,14,20,0.86); border-top: 1px solid rgba(80,255,150,0.14); border-right: 1px solid rgba(80,255,150,0.14); border-bottom: 1px solid rgba(80,255,150,0.14); }
...
export default function Admin() {
  ...
  const influence = [
    ['State', 24 + signalValues.length * 2],
    ['Memory', recallItems.length ? 28 : 8],
    ['Persona', personaPacket?.engaged ? 20 : 4],
    ['Pressure', pressure?.momentum === 'high' ? 14 : 9],
    ['Broker', Number(brokerPacket.volume || 0) ? 10 : 3],
    ['Delivery', 6],
  ];
  ...
  return (
    <div className="engine-admin" data-testid="admin-page">
      ...
      {viewMode === 'cockpit' && <>
        <div className="engine-section-label">Cockpit</div>
        <main className="engine-cockpit-grid">
          <section className="engine-hero"><div className="engine-hero-title">BRUNEL</div><div className="engine-hero-grid"><Stat label="State" value={statePacket.state || '—'} /><Stat label="Mode" value={statePacket.mode || '—'} /><Stat label="Pressure" value={pressure.momentum || pressure.state_bias || '—'} /><Stat label="Turn" value={runtimeLast?.turn_count ?? '—'} /><Stat label="Memory" value={memoryStatus?.writable ? 'Writable' : 'Check'} /><Stat label="Recall" value={recallItems.length ? `${recallItems.length} items` : 'Quiet'} /></div></section>
          <Card title="Engine Health" subtitle="Fast health strip for active runtime systems." wide tone="runtime" collapsible={false}>
            <div className="engine-health-strip"><HealthItem label="Memory" value={memoryStatus?.writable ? 'LIVE' : 'CHECK'} score={memoryStatus?.writable ? 100 : 35} /><HealthItem label="Recall" value={systemValues.recall_live_path ? 'LIVE' : 'OFF'} score={systemValues.recall_live_path ? 92 : 20} /><HealthItem label="Persona" value={systemValues.persona_packet_live_path ? 'LIVE' : 'OFF'} score={systemValues.persona_packet_live_path ? 96 : 20} /><HealthItem label="Pressure" value={pressure.momentum || 'stable'} score={pressure.momentum === 'high' ? 74 : 56} /><HealthItem label="Signals" value={signalValues.length} score={Math.min(100, signalValues.length * 16)} /><HealthItem label="Prompt" value={promptSize ? `${promptSize}` : 'ok'} score={promptScore} /></div>
          </Card>
          <Card title="Why This Response" subtitle="Estimated influence view from active runtime packets." tone="runtime" collapsible={false}>{influenceRows.map(([label, value]) => <InfluenceRow key={label} label={label} value={value} />)}</Card>
          <Card title="Current State" subtitle="Human-readable state, pressure, and tribunal view." tone="runtime" collapsible={false}><ReadRow label="State" value={statePacket.state} /><ReadRow label="Mode" value={statePacket.mode} /><ReadRow label="Zone" value={statePacket.zone} /><ReadRow label="Reason" value={statePacket.core_decision?.reason} /><ReadRow label="Sentinel" value={statePacket.tribunal?.sentinel ?? 0} /><ReadRow label="Arbiter" value={statePacket.tribunal?.arbiter ?? 0} /><ReadRow label="Empath" value={statePacket.tribunal?.empath ?? 0} /></Card>
          <Card title="Persona" subtitle="Active cartridge and expression-layer packet." tone="runtime" collapsible={false}><ReadRow label="Name" value={personaPacket.identity?.name || personaPacket.persona_id} /><ReadRow label="Role" value={personaPacket.identity?.role} /><ReadRow label="Mandate" value={personaPacket.identity?.mandate} /><ReadRow label="Directive" value={personaPacket.metadata?.directive_status} /></Card>
          ...
          <Card title="Packets" subtitle="Packet broker and prompt assembly status." tone="runtime" collapsible={false}><ReadRow label="Broker volume" value={brokerPacket.volume ?? 0} /><ReadRow label="Packet count" value={brokerPacket.packet_count ?? 0} /><ReadRow label="Prompt sections" value={Object.keys(promptSections || {}).length} /><ReadRow label="Runtime keys" value={(runtimeLast?.runtime_keys || []).join(', ')} /></Card>
        </main>
      </>}

      {viewMode === 'trace' && <>
        <div className="engine-section-label">Trace</div>
        <main className="engine-trace-grid">
          <TraceStep title="User Message" text={(promptWindow.user_message || '').slice(0, 260) || 'No captured user message.'} active={Boolean(promptWindow.user_message)} />
          <TraceStep title="Packet Broker" text={`${brokerPacket.packet_count ?? 0} packets. Volume ${brokerPacket.volume ?? 0}. ${(brokerPacket.notes || []).slice(0, 2).join(' ')}`} active={Boolean(runtime.packet_broker)} />
          <TraceStep title="Signal Analysis" text={Object.entries(statePacket.signals || {}).map(([key, value]) => `${key}: ${value}`).join(' · ') || 'No signal packet.'} active={Boolean(statePacket.signals)} />
          <TraceStep title="Tribunal" text={`Sentinel ${statePacket.tribunal?.sentinel ?? 0} · Arbiter ${statePacket.tribunal?.arbiter ?? 0} · Empath ${statePacket.tribunal?.empath ?? 0}`} active={Boolean(statePacket.tribunal)} />
          <TraceStep title="Pressure" text={`Momentum ${pressure.momentum || 'unknown'} · bias ${pressure.state_bias || 'unknown'} · trust ${pressure.trust ?? '—'} · elapsed ${pressure.elapsed_since_previous || pressurePacket.elapsed_silence || '—'}`} active={Boolean(runtime.pressure)} />
          <TraceStep title="Core Decision" text={`${statePacket.core_decision?.candidate_state || statePacket.candidate_state || 'candidate unknown'} → ${statePacket.core_decision?.final_state || statePacket.state || 'state unknown'} · ${statePacket.core_decision?.reason || 'no reason captured'}`} active={Boolean(statePacket.core_decision)} />
          <TraceStep title="Persona Packet" text={`${personaPacket.persona_id || 'BRUNEL'} · ${personaPacket.metadata?.directive_status || 'packet captured'} · prompt included: ${personaPacket.persona_packet_prompt_included ? 'yes' : 'no'}`} active={Boolean(personaPacket.engaged)} />
          <TraceStep title="Memory Recall" text={`${recallItems.length} selected · ${recallPacket.reason || 'no recall reason'} · summary first: ${recallPacket.limits?.summary_first ? 'yes' : 'no'}`} active={Boolean(recallPacket.metadata)} />
          <TraceStep title="Prompt Assembly" text={`${Object.keys(promptSections || {}).length} sections captured. Persona packet ${personaPacket.persona_packet_prompt_included ? 'included' : 'quiet'}. Recall ${memoryPacket.chronological_recall_prompt_included ? 'included' : 'quiet'}.`} active={Boolean(runtime.prompt_window)} />
          <TraceStep title="Response + Memory Write" text={`Turn ${runtimeLast?.turn_count ?? '—'}. Chronological write ${runtime.chronological_memory?.ok ? 'ok' : 'check runtime packet'}.`} active={Boolean(runtimeLast?.ok)} />
        </main>
      </>}
