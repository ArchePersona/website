...
.engine-admin-topbar {
  ...
  gap: 12px;
  border-bottom: 1px solid rgba(80,255,150,0.22);
  padding-bottom: 12px;
  margin-bottom: 12px;
}
...
.value-chip, .egress-chip, .seed-strip span, .tribunal-strip span {
  display: inline-flex;
...
@media (max-width: 900px) {
  .engine-admin { padding: 10px; }
  .engine-admin-topbar { flex-direction: column; }
  .engine-card, .engine-card-wide { grid-column: span 12; }
  .engine-two-col, .engine-meter-grid { grid-template-columns: 1fr; }
}
`;
...
function buildReconstructedModelPayload({ systemPrompt, history }) {
  const turns = Array.isArray(history) ? history : [];
  const lastTurn = turns.length ? turns[turns.length - 1] : null;
  const priorTurns = turns.length ? turns.slice(Math.max(0, turns.length - 1 - PROMPT_HISTORY_TURNS), -1) : [];
  const messages = [];
  ...
  return {
    ...
    history_turn_cap: PROMPT_HISTORY_TURNS,
...
function EngineMeter({ label, value }) {
  ...
  return (
    <div className="engine-meter">
      <div className="engine-meter-top"><span>{label}</span><span>{pct(numeric)}</span></div>
      <div className="engine-meter-track"><div className="engine-meter-fill" style={{ width: `${numeric * 100}%` }} /></div>
    </div>
  );
}
  ...
  return (
    <section className={`engine-card${wide ? " engine-card-wide" : ""}`}>
      <div className="engine-card-head">
        ...
        {actions && <div className="engine-card-actions">{actions}</div>}
      </div>
      <div className="engine-card-body">{children}</div>
    </section>
  );
}
...
export default function Admin() {
  ...
  const reconstructedModelPayload = buildReconstructedModelPayload({ systemPrompt: promptWindow.system_prompt, history });
  const seed = {
  ...
  const applyRuntime = async () => {
    ...
    setMsg("");
    try {
      await axios.post(`${API}/admin/runtime`, { state: pickState || null, mode: pickMode || null }, { headers: authHeader });
  ...
  const clearOverrides = async () => {
    ...
    setMsg("");
    try {
      await axios.post(`${API}/admin/runtime`, { state: "", mode: "" }, { headers: authHeader });
      setPickState("");
      setPickMode("");
  ...
  if (loading) return <div className="engine-admin"><style>{ENGINE_ADMIN_CSS}</style><div>loading engine console…</div></div>;

  if (forbidden) {
    return (
      <div className="engine-admin">
        <style>{ENGINE_ADMIN_CSS}</style>
        <div className="engine-brand-name">BRUNEL</div>
        <div className="engine-brand-sub">admin · forbidden</div>
        <p>Your account ({user?.email}) is not on the admin allowlist.</p>
        <button className="engine-btn" onClick={() => navigate("/brunel")}>Back to BRUNEL</button>
      </div>
    );
  }
  ...
  return (
    <div className="engine-admin" data-testid="admin-page">
      ...
      <header className="engine-admin-topbar">
        <div>
          <div className="engine-brand-name">BRUNEL</div>
          <div className="engine-brand-sub">admin · engine console</div>
        </div>
        <div className="engine-admin-actions">
          <button className="engine-btn" onClick={load} disabled={busy}>Refresh</button>
          <button className="engine-btn" onClick={() => copyText(safeJson(fullSnapshot), "snapshot")}>Copy Snapshot</button>
          <button className="engine-btn" onClick={downloadSnapshot}>Export JSON</button>
          <button className="engine-btn" onClick={() => navigate("/brunel")}>Back to BRUNEL</button>
          <button className="engine-btn engine-btn-dim" onClick={() => window.confirm("Sign out of BRUNEL?") && signOut()}>Sign out</button>
        </div>
      </header>

      {msg && <div className="engine-message">{String(msg)}</div>}

      <main className="engine-grid">
        <DataBlock title="System Values" subtitle="Fast read of the live engine state." wide>
          <div className="value-grid">
            <ValueChip label="turns" value={runtimeLast?.turn_count ?? 0} />
            <ValueChip label="persona" value={seed.persona} />
            <ValueChip label="state" value={seed.state} />
            <ValueChip label="mode" value={seed.mode} />
            <ValueChip label="zone" value={seed.zone} />
            <ValueChip label="broker" value={brokerPacket.volume ?? engineConfig?.packet_broker?.volume ?? 0} />
            <ValueChip label="influence" value={brokerPacket.influence ? "yes" : "no"} />
            <ValueChip label="prompt chars" value={(promptWindow.system_prompt || "").length} />
            <ValueChip label="payload msgs" value={reconstructedModelPayload.message_count} />
            <ValueChip label="runtime keys" value={(runtimeLast?.runtime_keys || []).length} />
          </div>
        ...
        <DataBlock title="Runtime Controls" subtitle="Admin override board.">
          ...
          <div className="engine-two-col">
            <label className="engine-field"><span>Forced State</span><input value={pickState} onChange={(e) => setPickState(e.target.value)} placeholder="natural" /></label>
            <label className="engine-field"><span>Forced Mode</span><input value={pickMode} onChange={(e) => setPickMode(e.target.value)} placeholder="natural" /></label>
          </div>
          <div className="engine-button-row">
            <button className="engine-btn engine-btn-primary" onClick={applyRuntime} disabled={busy}>{busy ? "Applying…" : "Apply Controls"}</button>
            <button className="engine-btn" onClick={clearOverrides} disabled={busy}>Clear State / Mode</button>
          </div>
        </DataBlock>

        <DataBlock title="Active Seed" subtitle="The current response seed reconstructed from runtime packets." actions={<button className="engine-mini-btn" onClick={() => copyText(safeJson(seed), "seed")}>Copy Seed</button>} wide>
          <div className="seed-strip"><span>persona · {seed.persona}</span><span>state · {seed.state || "—"}</span><span>mode · {seed.mode || "—"}</span><span>zone · {seed.zone || "—"}</span></div>
          <JsonPanel value={seed} maxHeight={260} />
        </DataBlock>

        <DataBlock title="Signal Meters" subtitle="Agent weights and tribunal levels coming out of the engine.">
          <div className="engine-meter-grid">{Object.entries(signals).map(([key, value]) => <EngineMeter key={key} label={key} value={value} />)}</div>
          <div className="tribunal-strip">{Object.entries(tribunal).map(([key, value]) => <span key={key}>{key} · {String(value)}</span>)}</div>
        </DataBlock>

        <DataBlock title="Packet Broker" subtitle="Last packet broker summary and influence report.">
          <div className="seed-strip"><span>volume · {brokerPacket.volume ?? engineConfig?.packet_broker?.volume ?? 0}</span><span>influence · {brokerPacket.influence ? "yes" : "no"}</span><span>packets · {(brokerPacket.packets || []).length}</span></div>
          <JsonPanel value={brokerPacket} maxHeight={260} />
        </DataBlock>

        <DataBlock title="Memory / Context" subtitle="Working memory, prompt memories, entropy, and presearch.">
          <div className="seed-strip"><span>working · {memoryPacket.working_memory || "—"}</span><span>external · {memoryPacket.external_memory_count ?? 0}</span><span>prompt memories · {(memoryPacket.prompt_memory_context || []).length}</span></div>
          <JsonPanel value={memoryPacket} maxHeight={320} />
        </DataBlock>

        <DataBlock title="PROMPT WINDOW" subtitle="Final assembled system prompt leaving Core for the model." actions={<button className="engine-mini-btn" onClick={() => copyText(promptWindow.system_prompt || "", "prompt")}>Copy Prompt</button>} wide>
          <div className="seed-strip"><span>stage · {promptWindow?.metadata?.capture_stage || "—"}</span><span>egress messages · reconstructed</span><span>chars · {(promptWindow.system_prompt || "").length}</span></div>
          <TextPanel value={promptWindow.system_prompt} />
        </DataBlock>

        <DataBlock title="Model Payload" subtitle="Reconstructed egress payload: system prompt plus saved message stack before latest answer." actions={<button className="engine-mini-btn" onClick={() => copyText(safeJson(reconstructedModelPayload), "model payload")}>Copy Payload</button>} wide>
          <JsonPanel value={reconstructedModelPayload} maxHeight={520} />
        </DataBlock>

        <DataBlock title="Prompt Sections" subtitle="PROMPT WINDOW sections extracted for inspection." wide>
          <JsonPanel value={promptSections} maxHeight={420} />
        </DataBlock>

        <DataBlock title="Full Runtime Packet" subtitle="Everything captured from the engine for the last turn." wide>
          <JsonPanel value={runtime} maxHeight={520} />
        </DataBlock>
      </main>
    </div>
  );
